...
class Master(object):
	...
	def run(self, n_iterations=1, min_n_workers=1, maxcost=1.0e6,alg_name='bohb',HF=True,prior=[],iteration_kwargs = {},gmma0=0.1,ns=5):
		"""
		...
		while True:
			...
			results = Result([copy.deepcopy(i.data) for i in self.iterations] + ws_data, self.config)
			a = results.data
			budget = [0]
			try:
				budget = []
				for key, entry in a.items():
					if len(entry.results.items())>0:
						budget.append(list(entry.results.items())[0][1]['info'])
			except:
				pass

			if alg_name!='bohb' and HF==False:
				try:
					results = Result([copy.deepcopy(i.data) for i in self.iterations] + ws_data, self.config)
					a = results.data
					...
					if len(a)>0:
						for a1 in a.values():
							if len(a1.results.values())>0:
								x = list(a1.config.values())
								fid1 = a1.budget
								X.append(x), fid.append(fid1), F.append(list(a1.results.values())[0]['loss'])
						fid=np.round(fid, 2)
						if len(X)>10 and len(np.unique(fid))==1:
							from task_similarity.cal_sim import  cal_cov
							X=np.array(X)
							cs=self.config_generator.configspace
							...
							# ns=5
							X2 = np.array(X1).T
							sim=cal_cov(X2, np.array(F),cs1,ns=ns)
							# print('iteration:',n_iterations,' sim:',sim[-1])
							if (len(sim)>len(cs) and np.abs(1-sim[-1])<gmma0) or  sum(budget)>maxcost:#n_iterations<0 or
								break
					# for k in a.keys():
					# 	cost.append(list(a[k].results.items())[0][1]['info'])
				except:
					pass
			next_run = None
			...
			for i in self.active_iterations():
				...
				if  len(prior)>0 and not next_run is None:
					for key in next_run[1].keys():
						next_run[1][key]=prior[key]
					prior=[]
			...
			if not next_run is None:
				...
				if sum(budget)>maxcost:	#and sum(budget)<maxcost we might be able to start the next iteration
					n_iterations=0
				if n_iterations :	#and sum(budget)<maxcost we might be able to start the next iteration
