...
class Master(object):
	...
	def run(self, n_iterations=1000, min_n_workers=1, iteration_kwargs = {},maxcost=None):
		"""
		...
		while True:
			...
			if not next_run is None:
				...
				runtime.append(sum(cost))
					
				# print('cost/maxcost:',sum(cost),'/',maxcost)
				# we might be able to start the next iteration
