...
class MF_Opt:
    ...
    def __call__(self):
        ...
        npop=100
        pop=init_latin_hypercube_sampling(problem_bounds[:, 0], problem_bounds[:, 1], npop,np.random.RandomState(Param.r))
        ...
        m.optimize()
        pop_t = np.hstack((pop, (fo.task-1)*np.ones((npop,1))))
        mean, var= m.predict(pop_t)
        fpop=mean
        ...
        NF = NonDominatedSorting()


        while runtime[-1] < maxcost and NFill < 30:
            pop_child=np.zeros((npop,dimension))
            for i in range(0, npop):
                idx_list = choice(list(set(range(0, npop)) - {i}), 2, replace=False)

                pos_new = pop[i,:]+ self.weighting_factor * (gbest- pop[i,:]) + \
                          self.weighting_factor * (pop[idx_list[0],:] - pop[idx_list[1],:])
                pos_new = where(uniform(0, 1, dimension) < self.crossover_rate, pos_new,pop[i,:] )
                pos_new =np.clip(pos_new, problem_bounds[:, 0], problem_bounds[:, 1])
                pop_child[i,:] = pos_new
            ...
            pop_t = np.hstack((pop1, (ind_task) * np.ones((pop1.shape[0], 1))))
            mean, var = m.predict(pop_t)
            obj_history=np.hstack((mean,-var))
            fronts = NF.do(np.array(obj_history))

            Xc = pop1[fronts[0], :]
            Yc = obj_history[fronts[0], :]
            ...
            for i in range(len(X_new)):
                ...
                pop_t = np.hstack((pop1, (fo.task - 1) * np.ones((pop1.shape[0], 1))))
                mean, var = m.predict(pop_t)
                fpop = mean
                fpop=mean[0:npop,0]
                fpop_child=mean[npop:,0]

                if math.isnan(max(mean)) == False:

                    pop = [pop[i,:] if fpop[i] < fpop_child[i] else pop_child[i,:] for i in range(npop)]
                    fpop =[fpop[i] if fpop[i] < fpop_child[i] else fpop_child[i] for i in range(npop)]
                    pop=np.array(pop)
                    fpop=np.array(fpop)
                    Ib=np.where(fpop==np.min(fpop))[0][0]
                    if fgbest>fpop[Ib]:
