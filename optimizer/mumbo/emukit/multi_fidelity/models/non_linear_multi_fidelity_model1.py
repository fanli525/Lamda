...
def make_non_linear_kernels(base_kernel_class: Type[GPy.kern.Kern],
                            n_fidelities: int, n_input_dims: int,  Ker_type,ARD: bool=False) -> List:
    """
    ...
    if Ker_type=='single':
    ...
    elif Ker_type=='l1_std':

        base_dims_list = list(range(n_input_dims))
        K11 = GPy.kern.StdPeriodic(n_input_dims, active_dims=base_dims_list, ARD1=ARD)  # My_Periodic
        K12 = GPy.kern.Matern52(n_input_dims)
        base_kernel1 = GPy.kern.Add([K11, K12], name='kern_fidelity_1')
        kernels = [base_kernel1]
        for i in range(1, n_fidelities):
            fidelity_name = 'fidelity' + str(i + 1)

            K22 = GPy.kern.Matern52(n_input_dims)
            base_kernel2 = GPy.kern.Add([ K22], name='scale_kernel_' + fidelity_name)
            interaction_kernel = base_kernel2


            K32 = GPy.kern.Matern52(1, active_dims=[n_input_dims], ARD=ARD, )
            base_kernel3 = GPy.kern.Add([ K32], name='previous_fidelity_' + fidelity_name)
            scale_kernel = base_kernel3

            K42 = GPy.kern.Matern52(n_input_dims)
            base_kernel4 = GPy.kern.Add([K42], name='bias_kernel_' + fidelity_name)
            bias_kernel = base_kernel4
            kernels.append(interaction_kernel * scale_kernel + bias_kernel)



    elif Ker_type=='error_std':
        ...
        for i in range(1, n_fidelities):
            ...
            K41 = GPy.kern.StdPeriodic(n_input_dims, active_dims=base_dims_list, ARD1=ARD, )  # My_Periodic
            K42 = GPy.kern.Matern52(n_input_dims)
            base_kernel4 = GPy.kern.Add([K41,K42], name='bias_kernel_' + fidelity_name)
            bias_kernel = base_kernel4
            kernels.append(interaction_kernel * scale_kernel + bias_kernel)
        ...
        for i in range(1, n_fidelities):
            ...
            K31 = GPy.kern.StdPeriodic(1, active_dims=[n_input_dims], ARD1=ARD, )  # My_Periodic
            K32 = GPy.kern.Matern52(1, active_dims=[n_input_dims], ARD=ARD, )
            base_kernel3 = GPy.kern.Add([K31, K32], name='previous_fidelity_' + fidelity_name)
            scale_kernel = base_kernel3
        ...
        for i in range(1, n_fidelities):
            fidelity_name = 'fidelity' + str(i + 1)
            K21 = GPy.kern.StdPeriodic(n_input_dims, active_dims=base_dims_list)  # My_Periodic
            K22 = GPy.kern.Matern52(n_input_dims)
            base_kernel2 = GPy.kern.Add([K21, K22], name='scale_kernel_' + fidelity_name)
            interaction_kernel = base_kernel2
        ...
        for i in range(1, n_fidelities):
            ...
            K32 = GPy.kern.Matern52(1, active_dims=[n_input_dims], ARD=ARD, )
            K33 = GPy.kern.Linear(1, active_dims=[n_input_dims], ARD=ARD, )
            base_kernel3 = GPy.kern.Add([K33,K32], name='previous_fidelity_' + fidelity_name)
            scale_kernel = base_kernel3
        ...
        for i in range(1, n_fidelities):
            ...
            K32 = GPy.kern.Matern52(1, active_dims=[n_input_dims], ARD=ARD, )
            K33 = GPy.kern.Linear(1, active_dims=[n_input_dims], ARD=ARD, )
            base_kernel3 = GPy.kern.Add([K32,K33], name='previous_fidelity_' + fidelity_name)
            scale_kernel = base_kernel3
    ...
    elif Ker_type=='flinear_error_std':
    ...
    elif Ker_type=='flinear_error_mystd':


        base_dims_list = list(range(n_input_dims))
        K11 = GPy.kern.My_Periodic(n_input_dims, active_dims=base_dims_list, ARD1=ARD)  # My_Periodic
        K12 = GPy.kern.Matern52(n_input_dims)
        base_kernel1 = GPy.kern.Add([K11,K12], name='kern_fidelity_1')
        kernels = [base_kernel1]
        for i in range(1, n_fidelities):
            ...
            K41 = GPy.kern.My_Periodic(n_input_dims, active_dims=base_dims_list, ARD1=ARD, )  # My_Periodic
            K42 = GPy.kern.Matern52(n_input_dims )
            base_kernel4 = GPy.kern.Add([K41,K42], name='bias_kernel_' + fidelity_name)
            ...
            bias_kernel = base_kernel4
            kernels.append(interaction_kernel * scale_kernel + bias_kernel)
            # kernels.append(interaction_kernel + bias_kernel )
        ...
        for i in range(1, n_fidelities):
            ...
            K32 = GPy.kern.Matern52(1, active_dims=[n_input_dims], ARD=ARD, )
            K33 = GPy.kern.Linear(1, active_dims=[n_input_dims], ARD=ARD, )
            base_kernel3 = GPy.kern.Add([K31,K32,K33], name='previous_fidelity_' + fidelity_name)
            scale_kernel = base_kernel3
...
class NonLinearMultiFidelityModel(IModel, IDifferentiable):
    ...
    def __init__(self, X_init: np.ndarray, Y_init: np.ndarray, n_fidelities, kernels: List[GPy.kern.Kern],
                 n_samples=100, verbose=False, optimization_restarts=5,GPtype='GP') -> None:
        """
        ...
        if self.GPtype=='GP':
            ...
            num_inducing=np.min([X_init[is_lowest_fidelity, :-1].shape[0],10*d])
            self.models.append(
                GPy.models.SparseGPRegression(X_init[is_lowest_fidelity, :-1], Y_init[is_lowest_fidelity, :], kernels[0],num_inducing=num_inducing))
        ...
        for i in range(1, self.n_fidelities):
            ...
            if self.GPtype=='GP':
                ...
                num_inducing=np.min([augmented_input.shape[0],10*d])

                self.models.append(GPy.models.SparseGPRegression(augmented_input, Y_init[is_ith_fidelity, :], kernels[i],num_inducing=num_inducing))
    ...
    def _propagate_samples_through_level(self, X, i_level, sample_mean, sample_variance):
        ...
        """
        # Draw samples from posterior of previous fidelity
        samples = self.monte_carlo_rand_numbers * np.sqrt(abs(sample_variance)) + sample_mean.T
        # 原文没有abs，为了避免预测方差为负，粗略取为
