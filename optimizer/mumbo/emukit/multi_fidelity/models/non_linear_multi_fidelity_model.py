...
def make_non_linear_kernels(base_kernel_class: Type[GPy.kern.Kern],
    ...
    if Ker_type=='single':
        ...
        # K11 = GPy.kern.Cosine(n_input_dims, active_dims=base_dims_list, ARD=ARD, ) 
        # 
        # K11 = GPy.kern.My_Periodic(n_input_dims )
        K11 = GPy.kern.StdPeriodic(n_input_dims)  # My_Periodic
        ...
        for i in range(1, n_fidelities):
            ...
            # K21 = GPy.kern.My_Periodic(n_input_dims )  # My_Periodic
            # K21 = GPy.kern.Cosine(n_input_dims)  #StdPeriodic My_Periodic
            K22 = GPy.kern.Matern52(n_input_dims)
            K23 = GPy.kern.Linear(n_input_dims)
            ...
            # K31 = GPy.kern.ExpQuadCosine(1, active_dims=[n_input_dims], ARD=ARD, ) 
            # K31 = GPy.kern.My_Periodic(1, active_dims=[n_input_dims], ARD1=ARD, )
            # K31 = GPy.kern.Cosine(1)
            ...
            # K41 = GPy.kern.Cosine(n_input_dims )  # My_Periodic
            K42 = GPy.kern.Matern52(n_input_dims)
            K43 = GPy.kern.Linear(n_input_dims, active_dims=base_dims_list, ARD=ARD, )
            
            K41 = GPy.kern.StdPeriodic(n_input_dims, active_dims=base_dims_list )  # My_Periodic
            # K42 = GPy.kern.Matern52(n_input_dims, active_dims=base_dims_list )
            # K43 = GPy.kern.Linear(n_input_dims, active_dims=base_dims_list )
...
class NonLinearMultiFidelityModel(IModel, IDifferentiable):
    ...
    def __init__(self, X_init: np.ndarray, Y_init: np.ndarray, n_fidelities, kernels: List[GPy.kern.Kern],
                 n_samples=100, verbose=False, optimization_restarts=5,normalizer=False) -> None:
        """
        ...
        self.models.append(
            GPy.models.GPRegression(X_init[is_lowest_fidelity, :-1], Y_init[is_lowest_fidelity, :], kernels[0],normalizer=self.normalizer))
        ...
        for i in range(1, self.n_fidelities):
            ...
            self.models.append(GPy.models.GPRegression(augmented_input, Y_init[is_ith_fidelity, :], kernels[i],normalizer=self.normalizer))
