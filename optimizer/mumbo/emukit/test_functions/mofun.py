...
def multi_fidelity_forrester_function(Tbd, Tbu,d,high_fidelity_noise_std_deviation=0, low_fidelity_noise_std_deviation=0):
    """
    ...
    """
    space = [ContinuousParameter('x'+str(i), Tbd[i], Tbu[i]) for i in range(d)]
    s2=space.append(InformationSourceParameter(2))
    s2=space
    parameter_space = ParameterSpace(s2)
    # parameter_space = ParameterSpace([[xi for xi in space],InformationSourceParameter(2)])
