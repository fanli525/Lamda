...
class per_MB(Problem):   # 利用单输出的预测不确定计算EI
    ...
    def __init__(self, n_var,n_obj,xl,xu, model, N_train,cost,d,w,high_gp_model,ind_task,var_H,**kwargs):
...
class SGP_DiscretEI:
    def __init__(self, model, d,d_int,d_log=[]):
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        ...
        if len(self.d_log)>0:
            ind=np.intersect1d(self.d_log,self.d_int)
            ind1 = np.setdiff1d(self.d_int,ind)

            if len(ind)>0:
                x[:,ind]=np.round(np.exp(x[:,ind]))
                x[:,ind]=np.log(x[:,ind])
            if len(ind1)>0:
                x[:,ind1] = np.round(x[:,ind1])        
        else:
            x[:,self.d_int]=np.round(x[:,self.d_int])
...
class SGP_DiscretLCB:
    def __init__(self, model, d,d_int,d_log=[],w=2):
