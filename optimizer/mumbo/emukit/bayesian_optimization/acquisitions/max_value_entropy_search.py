...
from ...core import InformationSourceParameter,InformationSourceParameter_fixvalue
...
class MaxValueEntropySearch(Acquisition):
    ...
    def update_parameters(self):
        ...
        gumbel_samples[gumbel_samples>left-np.mean(fsd)]=left-np.mean(fsd)        
...
def _fit_gumbel(fmean, fsd):
    ...
    left = np.min(fmean - 5 * fsd)
    right = np.max(fmean + 5 * fsd)
    left = min(fmean - 5 * fsd)
    right = max(fmean + 5 * fsd)
...
def _find_source_parameter(space):
    ...
    for i, param in enumerate(space.parameters):
        if isinstance(param, InformationSourceParameter) or  isinstance(param, InformationSourceParameter_fixvalue):
...
class ACMBO(MaxValueEntropySearch):
    def __init__(self, model: Union[IModel, IEntropySearchModel], space: ParameterSpace,
                 target_information_source_index: int = None, num_samples: int = 10,
                 grid_size: int = 5000,AC='LCB') -> None:
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.AC=='LCB':
        # build MUMBO acquisition function
            f_acqu_x =-fmean+2*np.sqrt(fvar)

            f_acqu_x=np.log(1+np.exp(f_acqu_x))

        elif self.AC=='EI':

            y11 = self.model.Y[self.model.X[:,-1]==x[0,-1]]

            fmin = min(y11)
            Z = (fmin - fmean) / sigma
            ei= (fmin - fmean) * norm.cdf(Z) + sigma * norm.pdf(Z)
            f_acqu_x=ei
