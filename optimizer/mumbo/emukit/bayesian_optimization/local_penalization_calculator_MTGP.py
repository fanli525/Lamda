...
class LocalPenalizationPointCalculator_MTGP(CandidatePointCalculator):
    """
    ...
    def compute_next_points(self, loop_state: LoopState, context: dict=None) -> np.ndarray:
        ...
        for _ in range(self.batch_size):
            ...
            if self.parameter_space.parameters[-1].domain[0] == 0:
                f_min =np.min(self.model.Y[self.model.X[:,-1]==0])
            elif self.parameter_space.parameters[-1].domain[0] == 1:
                f_min =np.min(self.model.Y[self.model.X[:,-1]==1])
            else:
...
def _estimate_lipschitz_constant(space: ParameterSpace, model: IDifferentiable):
    ...
    if space.parameters[-1].domain[0] == 0:
        samples = np.vstack([samples, model.X[model.X[:, -1] == 0,:]])

    elif space.parameters[-1].domain[0] == 1:
        samples = np.vstack([samples, model.X[model.X[:, -1] == 1,:]])
    else:
        samples = np.vstack([samples, model.X])
