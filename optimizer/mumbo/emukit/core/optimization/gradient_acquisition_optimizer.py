...
from .optimizer import OptLbfgs, OptTrustRegionConstrained, apply_optimizer,OptDE
...
class GradientAcquisitionOptimizer(AcquisitionOptimizerBase):
    ...
    """
    def __init__(self, space: ParameterSpace, num_samples=1000, num_anchor=10,DE=False) -> None:
        """
    ...
    def _optimize(self, acquisition: Acquisition, context_manager: ContextManager) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.DE == True:
            ...
            optimized_point = apply_optimizer(optimizer, np.zeros(context_manager.space.dimensionality) , space=self.space, f=f, df=None, f_df=f_df,
    ...
    def _get_optimizer(self, context_manager):
        ...
        if self.DE == True:
            return OptDE(context_manager.contextfree_space.get_bounds())
