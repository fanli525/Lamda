...
class OptDE(Optimizer):
    """
    Wrapper for l-bfgs-b to use the true or the approximate gradients.
    """

    def __init__(self, bounds, max_iterations=500):
        super(OptDE, self).__init__(bounds)
        self.max_iterations = max_iterations
    ...
    def optimize(self, x0: np.ndarray, f: Callable=None, df: Callable=None, f_df: Callable=None) \
        ...
        result = scipy.optimize.differential_evolution(f, bounds=self.bounds, maxiter=self.max_iterations)
