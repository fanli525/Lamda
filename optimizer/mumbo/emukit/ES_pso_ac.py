...
from emukit.bayesian_optimization.acquisitions import MaxValueEntropySearch,EntropySearch
...
import copy
class Sgp_lcbpso: 
    def __init__(self,dimension,problem_bounds,fo,N_train0,maxN,AC_name,r):
        self.dimension =dimension
        self.problem_bounds =problem_bounds
        self.fo =fo
        self.N_train0=N_train0
        self.maxN = maxN
    ...
    def __call__(self):
        ...
        # N_train=10
        N_train0=self.N_train0 
        N_train=copy.deepcopy(N_train0)
        d=dimension
        # 初始化模型的参数
        ...
        u=init_latin_hypercube_sampling(Tbd, Tbu, n,np.random.RandomState(self.r))
        u=0.25*u; v=u;    
        lbest=pop
        it=0
        lbest=copy.deepcopy(pop);
        gbest=lbest[0,:]
        ...
        while N_train<=maxN and Tc==1:#  
           ...
           # 由大到小排序
            ...
            if N_new==0:
                ...
                Ib = np.where(var == np.max(var))[0][0]
                x_new=pop[Ib,:]
        ...
        EV_inf.append({'D_diver': D_diver.tolist(),'Ac_px': Ac_px.tolist()})
