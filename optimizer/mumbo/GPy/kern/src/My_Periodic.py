# -*- coding: utf-8 -*-
"""
Created on Sat Sep 18 16:50:12 2021
...
class My_Periodic(Kern):
    ...
    :type Boolean
    """
    def __init__(self, input_dim, variance=1., period=None, lengthscale=None, ARD1=False, ARD2=False, active_dims=None, name='My_periodic',useGPU=False):
        super(My_Periodic, self).__init__(input_dim, active_dims, name, useGPU=useGPU)
    ...
    def to_dict(self):
        ...
        input_dict = super(My_Periodic, self)._save_to_input_dict()
        input_dict["class"] = "GPy.kern.My_Periodic"
    ...
    def K(self, X, X2=None):
        ...
        dist =cdist(X, X2,metric='euclidean')
        dist=np.pi*dist/2
        e1=-2*(np.sin(dist))**2/(self.lengthscale**2)
        return self.variance* np.exp(e1) #sigma_f**2 * np.exp(e1)      
            
        # base = np.pi * dist[:,:,None] / self.period

        # # base = np.pi * (X[:, None, :] - X2[None, :, :]) 
