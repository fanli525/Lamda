...
class myStdPer1(Kern):
    ...
    def __init__(self, input_dim, variance=1., period=None, lengthscale=None, ARD1=False, ARD2=False, active_dims=None,
                 name='std_periodic', useGPU=False):
        super(myStdPer1, self).__init__(input_dim, active_dims, name, useGPU=useGPU)
    ...
    def to_dict(self):
        ...
        input_dict = super(myStdPer1, self)._save_to_input_dict()
        input_dict["class"] = "GPy.kern.myStdPer1"
    ...
    def K(self, X, X2=None):
        ...
        r = cdist(X, X2,metric='euclidean')
        base = np.pi * (r) / self.period


        # base = np.pi * (X[:, None, :] - X2[None, :, :]) / self.period
        exp_dist = np.exp(-0.5 * np.square(np.sin(base) / self.lengthscale))
    ...
    def update_gradients_full(self, dL_dK, X, X2=None):
        ...
        exp_dist = np.exp(-0.5 *np.square(sin_base / self.lengthscale))
